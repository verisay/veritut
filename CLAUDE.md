...
| Yol | İçerik | Dev port |
| --- | --- | --- |
| `apps/api` | Express 5 + Drizzle + ws — REST `/api/v1`, OIDC oturum, `/internal`, `/ext` (public API), `/webhooks`, WS canlı log | 4400 |
...
| `packages/validators` | Zod şemaları (api + frontend ORTAK) + blueprint `inputs` → Zod derleyicisi | |
...
17. **Sipariş kotası SİPARİŞ EDİLEN planla hesaplanır** (`featuresForOrder`): aboneliksiz kiracı `free` planda olduğu için aksi hâlde ilk siparişini asla veremez (K3 smoke bulgusu). Aynı mantık SLA kapısında da geçerli.
18. **Webhook router'ı `express.json`'dan ÖNCE mount edilir** (`app.ts`) ve `raw({type:'*/*'})` kullanır; HMAC ham gövde üzerinden doğrulanır. Sıra bozulursa imza her zaman geçersiz olur.
19. **Core fatura KESMEZ (D17).** `pushPeriod` kalemleri omurgaya gönderir, fatura tablosuna yazan tek yol `applyBillingEvent`'tir (webhook ve push aynı yolu kullanır, idempotent upsert).
...
- **K3 — Ticari katman ve lansman: TAMAM (2026-09-06).** `0004` (13 tablo: katalog/fiyat/plan/SLA, sipariş, abonelik aynası, entitlement istisnası, kullanım, fatura, ticket, KPI, UTM). **Katalog**: ürün = satış birimi, blueprint = teknik tanım; yayımlanan TEK sürüm self-servise açılır. **Entitlement** tek formül (`getEffectiveFeatures`), Redis 5 dk cache, fail-closed kapılar + kota (iş yükü, kullanıcı, API anahtarı, SLA katmanı). **Self-servis sipariş**: fiyat teklifi → sözleşme onayı → provizyon → abonelik + kullanım kalemi + fatura, insan dokunmadan; 14 gün deneme (kiracı başına bir, en küçük boyut, cron ile askıya alma). **Faturalama** `IBillingProvider` (mock + FOSSBilling adaptörü): Core para toplamaz, fatura webhook aynasıyla gelir, ödeme abonelik makinesini ilerletir, askı iş yükünü durdurur (veri silinmez). **Destek** `ITicketProvider` (mock + Zammad): portal ticket + ajan yanıtı webhook'u + destek dakikası KPI'ı. **Public API** `/ext` (Bearer `vt_…`, kapsam + IP allow-list + 120 istek/dk + OpenAPI). **Pazarlama**: ana sayfa, ürün landing'leri (csr=false, Product/FAQPage JSON-LD), karşılaştırma sayfaları, fiyatlandırma, SLA, KVKK, sözleşmeler, geliştirici, kalite süzgeçli sitemap + robots. **KPI**: MRR, brüt marj, destek dk/müşteri, churn. Smoke `smoke-k3.mjs` 69 senaryo; toplam 226 yeşil. Borç: FOSSBilling/Zammad canlı credential, e-posta SMTP, Lighthouse ölçümü, kanıt paketi PDF (K4).
- **K4 — Güvence katmanı: TAMAM (2026-09-06).** `0005` (14 tablo: alarm, olay + güncelleme + iş yükü bağı, bakım, nöbet, alarm kanalı, SLA dönemi, tatbikat, belge, kanıt paketi, denetçi bağlantısı, tatil takvimi). **Olay**: Alertmanager webhook (bearer) → alarm → otomatik olay (critical→sev2, fırtına tek olaya düşer, müşteriye kapalı doğar); yaşam döngüsü + SLA saati (yanıt/çözüm hedefi, ciddiyet çarpanı, müşteri beklerken duran saat, T-15 eskalasyon, sev1/sev2 post-mortem zorunlu). **Nöbet**: rota + 2 seviye eskalasyon; nöbetçi yoksa tüm operatörlere düşer. **SLA motoru**: uptime sondalardan, planlı bakım düşülür, kredi tablosu koddan (`creditPctFor`), kredi talep beklemeden negatif fatura kalemi. **Tatbikat**: aylık takvim → runner `drill` → gerçek restic restore + checksum + veri kontrolü → `restore.drill.passed|failed` kanıtı; başarısızlık müşteriye bildirilir. **Sapma**: gece `tofu plan` (apply YOK) → `drift_reports` → ops onayı. **Belgeler** (pdfkit + gömülü DejaVu): alt işleyen listesi (veriden), DPA, aylık SLA raporu, kanıt paketi (PDF + JSON eki, SHA-256, zincir doğrulaması). **Denetçi bağlantısı**: süreli, kapsamlı, kimliksiz salt-okuma; belge indirme; iptal edilebilir. **Alarm kanalları**: webhook/Slack/Teams/e-posta/SMS, hedef şifreli, HMAC imzalı gönderim. Cron 13 job. Smoke `smoke-k4.mjs` 62 senaryo; toplam 288 yeşil. ISO 27001 eşlemesi: `docs/iso27001-eslesme.md`. Borç: Teleport CE kurulumu (uç hazır), Loki, prod realm MFA zorunluluğu, hukuk onayı.
- **K5 — Genişleme ve dogfooding:** sırada (plan §14 K5).
