...
| `infra/blueprints/<slug>/<semver>/` | IaC ürün tanımları (K2'de dolar) — DB yalnız referans tutar | |
...
12. **`awaiting_approval` yalnız `running`'den** geçilir; yüksek riskli çalıştırma onaylayan ≠ talep eden (dört-göz) — servis zorlar (K2).
...
- **K2 — Provizyon motoru:** sırada (plan §14 K2).
