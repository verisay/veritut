...
locals {
  ...
  extra_ports = compact(split(",", var.extra_ports))
}
...
resource "hcloud_firewall" "fw" {
  ...
  labels = local.labels
  dynamic "rule" {
    for_each = toset(concat(["22", "80", "443"], local.extra_ports))
    content {
      direction  = "in"
      protocol   = "tcp"
      port       = rule.value
...
output "probe_url" { value = "http://${hcloud_server.vm.ipv4_address}:9100/metrics" }
output "endpoints" {
  value = [{ label = "SSH", url = "ssh://root@${var.hostname}.${var.domain}" }]
}
output "access" {
